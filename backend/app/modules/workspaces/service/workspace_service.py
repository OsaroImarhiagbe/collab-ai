...
class WorkSpaceService:
    ...
    async def create_workspace(self,name:str,user_id:UUID) -> WorkSpaceResponse:
        """
        function will handle create the workspace
        """

        ## Step 1 : Check to see if we already have the workspace created
        workspace_exists = await self.__repo.grab_workspace_from_db(name=name)
        ...
        workspace_results = await self.__repo.create_workspace_in_db(name=name,user_id=user_id)
