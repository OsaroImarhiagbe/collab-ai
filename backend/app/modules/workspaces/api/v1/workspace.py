...
async def create_workspace(request:WorkSpaceRequest,service: get_workspace_service_dependency,current_user:get_current_user_dependency) -> WorkSpaceResponse:
    try:
        response = await service.create_workspace(name=request.name,user_id=request.user_id)
        return response
